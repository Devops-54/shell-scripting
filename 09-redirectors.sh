...
Redirectors are of 2 types in bash
    ...
    2)Output Redirector  ( Means routing the output to a file )  :   >       >>  ( >> appends the latest output to the existing content)
...
# 0         :  successful 
# 1-255     :  partial failure or complete failure or partial sucess. 
